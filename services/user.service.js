...
export async function checkMissingFields(userId) {
  ...
  const totalRequiredFields =
    Object.values(fieldCategories).flat().length +
    requiredAddressFields.length +
    requiredEducationFields.length +
    requiredProfessionalFields.length +
    1; // profilePic

  let missingCount = 0;

  Object.values(missingFields).forEach((fields) => {
    if (Array.isArray(fields)) {
      missingCount += fields.length;
    }
  });

  const completedFields = totalRequiredFields - missingCount;
...
export async function checkMissingFieldsMobile(userId) {
  const fieldCategories = {
    basicDetails: ['writeBoutYourSelf', 'height', 'weight', 'caste', 'religion', 'maritalStatus', 'gender'],
    contactDetails: ['email', 'mobileNumber'],
    hobbiesAndInterest: ['hobbies'],
  };

  const requiredLocationDetailsFields = ['currentCity', 'currentCountry', 'currentState'];
  const requiredEducationDetailsFields = ['degree', 'collage'];
  const requiredProfessionalFields = ['jobTitle', 'jobType', 'companyName', 'currentSalary', 'workCity', 'workCountry'];

  const user = await User.findById(userId).populate('address').populate('userEducation').populate('userProfessional').exec();
  ...
  if (!user.hobbies || user.hobbies.length === 0) {
    missingFields.hobbiesAndInterest = ['hobbies'];
  }
  ...
  if (user.address) {
    ...
    missingFields.locationDetails = requiredLocationDetailsFields;
  }
  ...
  if (user.userEducation) {
    ...
    missingFields.educationDetails = requiredEducationDetailsFields;
  }
  ...
  if (user.userProfessional) {
    ...
    missingFields.professional = requiredProfessionalFields;
  }
  ...
  const totalRequiredFields =
    Object.values(fieldCategories).flat().length +
    requiredLocationDetailsFields.length +
    requiredEducationDetailsFields.length +
    requiredProfessionalFields.length +
    1; // profilePic
